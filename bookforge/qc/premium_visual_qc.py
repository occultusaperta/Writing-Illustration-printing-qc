...
def run_premium_visual_qc(
    ...
    *,
    ...
    for idx, path in enumerate(selected_pages, start=1):
        ...
        rows.append(row)
        if fails:
            weak.append({"page": idx, "score": score, "failures": fails})

    weak_sorted = sorted(weak, key=lambda x: x["score"])[:8]
    hard_fail_pages = [x["page"] for x in rows if x["score"] < 0.74]
    status = "FAIL" if hard_fail_pages else "PASS"
    regen = [{"page": p, "reason": "premium_qc_score_below_threshold"} for p in hard_fail_pages]
