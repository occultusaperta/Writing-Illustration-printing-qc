...
def score_baby_schema(image_path: str | Path, *, generalized_mode: bool = True) -> BabySchemaScoreResult:
    ...
    head_to_body_ratio_score = clamp01(0.35 + (top_var / max(bottom_var + 1e-6, 1.0)) * 0.4)
    eye_prominence_score = clamp01(0.2 + 2.6 * top_dark + 0.8 * top_edge)
    ...
    if len(xs) > 0:
        ...
        roundness_proxy = 0.1
    face_roundness_score = clamp01(roundness_proxy * 2.5)

    cheek_fullness_or_softness_score = clamp01(1.0 - soft_grad / 34.0)

    low_sat_ratio = float((sat < np.quantile(sat, 0.6)).mean())
    vertical_extent = float((gray < np.quantile(gray, 0.4)).mean())
    limb_shortness_softness_score = clamp01(0.55 * low_sat_ratio + 0.45 * (1.0 - vertical_extent))
    ...
    composite = clamp01(0.7 * overall + 0.3 * face_roundness_score)
    ...
    confidence = clamp01(0.25 + min(0.5, (top_var + bottom_var) / 110.0) + min(0.25, soft_patch.size / max(gray.size, 1)))
