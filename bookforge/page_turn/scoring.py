...
def score_page_turn_tension(
    ...
    *,
    ...
    rightward_vector_score = _clamp01(0.45 + 0.9 * (right_energy_ratio - 0.5))
    cropped_continuation_score = _clamp01(0.35 + (right_edge_density / max(right_edge_density + left_edge_density, 1e-6)))
    ...
    incomplete_action_score = _clamp01(0.25 + 0.45 * saliency_comp + 0.2 * fixation + action_hint)
    ...
    lighting_pull_score = _clamp01(0.5 + 1.4 * light_delta)
    ...
    left_pull = _clamp01((left_energy - right_energy) / max(total_energy, 1e-6) + 0.5)
    central_deadness = _clamp01(0.55 - center_std / 32.0)
    turn_resistance_penalty = _clamp01(0.4 * left_pull + 0.25 * central_deadness + closure_bias)
    ...
    confidence = _clamp01(0.45 + 0.12 * sum(1 for v in evidence if 0.2 <= v <= 0.95))
